...
def Q_i(Q_min, i, j, k):
    """Construct the Q_t matrix by left-top paddin the matrix Q with elemens from the identity matrix."""
    if i < k or j < k:
        return float(i==j)
    else:
...
def householder(A):
    """Performs a Householder Reflections based QR Decomposition of the matrix A. The function returns Q, an orthogonal matrix and R, an upper triangular matrix such that A = QR."""
    n = len(A)
    ...
    # Since Q is defined as the product of transpose of Q_t,
